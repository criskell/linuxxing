...
const tokenizeStep = (accumulator: TokenizeAccumulator, character: string): TokenizeAccumulator => {
  if (accumulator.quote) {
    ...
    if (accumulator.substitutionDepth > 0) {
      return { ...accumulator, quote: closesQuote ? null : accumulator.quote, current: accumulator.current + character };
    }
    return closesQuote
  ...
  if (accumulator.pendingDollar) {
    if (character === '(') {
      return { ...accumulator, current: accumulator.current + '$(', substitutionDepth: accumulator.substitutionDepth + 1, pendingDollar: false };
    }
  ...
  if (accumulator.substitutionDepth > 0) {
    ...
    if (character === ')') {
      return { ...accumulator, substitutionDepth: accumulator.substitutionDepth - 1, current: accumulator.current + character };
    }
  ...
  if (character === ' ') {
    ...
    return { tokens: [...accumulator.tokens, accumulator.current], current: '', quote: null, substitutionDepth: 0, pendingDollar: false };
  }
...
export const tokenize = (input: string) => {
  const initial: TokenizeAccumulator = { tokens: [], current: '', quote: null, substitutionDepth: 0, pendingDollar: false };
