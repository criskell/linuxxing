...
export const createCommandTokenState = () => ({ subcommandClaimed: false, pendingValueFlag: null }) as CommandTokenState;
...
const classifyRedirect = (token: string, state: CommandTokenState) => {
  ...
  return { classification: { type: 'redirect' as const }, state };
};
...
const classifyLoneDash = (token: string, knowledgeBase: CommandDef | undefined, state: CommandTokenState) => {
  ...
  const classification = known
    ...
    : { type: 'arg' as const, consumedValueFlag: null, keyValueMatch: null };
...
const classifyArgument = (
  ...
  return { classification: { type: 'arg' as const, consumedValueFlag, keyValueMatch }, state };
};
